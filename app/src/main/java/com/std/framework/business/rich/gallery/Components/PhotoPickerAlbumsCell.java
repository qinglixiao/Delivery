...
package com.std.framework.business.rich.gallery.Components;
...
public class PhotoPickerAlbumsCell extends FrameLayout {
    ...
    private class AlbumView extends FrameLayout {
        ...
        public AlbumView(Context context) {
            ...
            selector.setBackgroundResource(R.drawable.album_list_selector);
            addView(selector,
                    LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT));
        }
        ...
        if (albumEntry != null) {
            ...
            if (albumEntry.coverPhoto != null && albumEntry.coverPhoto.path != null) {
                ...
                if (albumEntry.coverPhoto.isVideo) {
                    albumView.imageView.setImage(
                            "vthumb://" + albumEntry.coverPhoto.imageId + ":"
                                    + albumEntry.coverPhoto.path,
                            null, getContext().getResources().getDrawable(R.drawable.album_nophotos));
                } else {
                    albumView.imageView.setImage(
                            "thumb://" + albumEntry.coverPhoto.imageId + ":"
                                    + albumEntry.coverPhoto.path,
                            null, getContext().getResources().getDrawable(R.drawable.album_nophotos));
                }
            } else {
                albumView.imageView.setImageResource(R.drawable.album_nophotos);
            }
            albumView.nameTextView.setText(albumEntry.bucketName);
            albumView.countTextView.setText(String.format(Locale.CHINA,"%d", albumEntry.photos.size()));
        } else {
            albumViews[a].setVisibility(INVISIBLE);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (AndroidUtilities.isTablet()) {
            itemWidth = (AndroidUtilities.dp(490) - ((albumsCount + 1) * AndroidUtilities.dp(4)))
                    / albumsCount;
        } else {
            itemWidth = (AndroidUtilities.displaySize.x
                    - ((albumsCount + 1) * AndroidUtilities.dp(4))) / albumsCount;
        }
        ...
        super.onMeasure(widthMeasureSpec, MeasureSpec
                .makeMeasureSpec(AndroidUtilities.dp(4) + itemWidth, MeasureSpec.EXACTLY));
    }
}
