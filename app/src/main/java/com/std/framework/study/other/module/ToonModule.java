...
public class ToonModule extends AbstractModule {
    ...
    interface toonModule {
        ...
        Observable<String> getCommonList(@Query("startId") int startId, @Query("line") int line, @Query("feedId") String feedId, @Query("endId") int endId, @Query("pageIndex") int pageIndex);

        @POST("http://api.feed.systoon.com/user/obtainFeedList")
        Observable<String> obtainFeedList(@Body TNPFeedInputForm inputForm);

    }

    public Observable<String> getCommonList() {
        return module.getCommonList(0, 20, "o_1472486901507314", 0, 0);
    }

    public Observable<String> obtainFeedList() {
