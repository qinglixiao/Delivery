package com.std.framework.comm.widget.slidexpandlistview;
...
import com.std.framework.comm.widget.slidexpandlistview.ActionSlideExpandableListView.OnExPandListener;
				...
				if (a != null && a.hasStarted() && !a.hasEnded()) {
					...
					if (type == ExpandCollapseAnimation.EXPAND) {
						if(mOnExPandListener != null){
							mOnExPandListener.onExpand(target, position);
//							target.measure(MeasureSpec.makeMeasureSpec(LayoutParams.WRAP_CONTENT, MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(LayoutParams.WRAP_CONTENT, MeasureSpec.UNSPECIFIED));
						}
